...
using Automation.Plugins.Share.ManualSupply.Model;
using Automation.Plugins.Share.ManualSupply.Options.Controls;
...
namespace Automation.Plugins.Share.ManualSupply.Options
{
    public class LocalDatabaseConnectionOption : AbstractOption
    {
        ...
        {
            base.Initialize();
            this.NodeName = "ShareManualSupplyDatabaseConnectionOption";
            this.Caption = "手工补货器数据库连接";
            this.Order = 1;
            this.ParentNodeName = "ShareManualSupplyParameterOption";
            this.NodeImage = Resource.database_16;
        ...
        private void LoadInfo()
        {
            ...
            {
                Logger.Error(string.Format("加载手工补货数据库连接参数失败！原因：{0}", ex.Message));
                XtraMessageBox.Show("加载手工补货数据库连接参数失败", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (CheckInformation())
            {
                ...
                {
                    Logger.Error(string.Format("保存手工补货数据库连接参数失败！原因：{0}", ex.Message));
                    XtraMessageBox.Show("保存手工补货数据库连接参数失败", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
