...
go
ALTER PROCEDURE [dbo].[pro_get_new_task_for_abnormity_stock_out] 
...
BEGIN	
	...
	set @empty_position = cursor for
		...
		where position_type = '04'    /*异形烟出库位置*/
			and has_goods = 0
	...
	begin
		...
		where a.[state] = '01'			 			/*任务等待中*/
			and e.position_type != '04'				/*当前位置不是异形烟出库位置*/
			...
			and e.position_type = '04'				/*目标位置是异形烟出库位置*/
	end
