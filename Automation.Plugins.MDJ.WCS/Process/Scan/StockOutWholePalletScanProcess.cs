...
{
    public class StockOutWholePalletScanProcess : AbstractProcess
    {
        ...
        public override void Execute()
        {
            try
            {
                ...
                if (state == null || state.ToString() != "StockOutWholePalletScan") return;
                ...
                StockOutDialog stockOutDialog = new StockOutDialog(taskID);
                if (stockOutDialog.ShowDialog() == DialogResult.OK)
                {
                    RestClient rest = new RestClient();
                    taskID = rest.FinishStockOutTask(taskID, stockOutDialog.StockOutQuantity);
                    if (taskID > 0)
                    {
            ...
            {
                Logger.Error("StockOutWholePalletScanProcess 出错，原因：" + ex.Message + "/n" + ex.StackTrace);
            }
        }
    }
}
